...
const Counter = ({ setProducts, pcs, item, products }: CounterProps) => {
  ...
  return (
    <div className="flex h-[15px] xs:h-[26px] w-[50px] xs:w-[88px] items-center">
      <button
        className="w-[15px] xs:w-[20px] border border-solid border-lenssisStroke "
        onClick={() => countHandler(-1)}
      >
        &nbsp;-&nbsp;
      </button>
      <div className="w-[20px] xs:w-[30px] xs:h-[26px] text-center border-y border-solid border-lenssisStroke text-sm flex items-center justify-center text-[7px]">
        {item.pcs}
      </div>
      <button
        className="w-[15px] xs:w-[20px] border border-solid border-lenssisStroke "
