...
const updateUserProfile = async (editUserForm:EditFormValueType):Promise<EditFormValueType | null> => {
  ...
  const {data} = await axiosInstance.put<AxiosResponse<EditFormValueType>>('/member/updateInfo',editUserForm,
  {
...
const useEditProfile = () => {
  ...
  const {mutate:editProfile} = useMutation((newUserProfile:EditFormValueType) => updateUserProfile(newUserProfile),{
    onSuccess: () => {
      fireToast({
        id:'업데이트 성공',
