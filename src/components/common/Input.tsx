  ...
  if(double){
    return (
      <div className='flex flex-col justify-center my-4 w-full h-fit gap-2'>
       ...
       <input name={name} onChange={onChange} value={value} onBlur={onBlur} className={`grow pl-[15px] text-sm w-full h-10 py-2 border border-solid border-lenssisStroke rounded-[5px] max-w-${inputWidth}  focus:outline-1 focus:outline-[#ABC8DF] placeholder-gray-400/60 `} type={type} placeholder={placeholder} readOnly={readonly} />
       <input name={name2} onChange={onChange}value={value2} onBlur={onBlur} className={`grow pl-[15px] text-sm w-full h-10 py-2 border border-solid border-lenssisStroke rounded-[5px] max-w-${inputWidth}  focus:outline-1 focus:outline-[#ABC8DF] placeholder-gray-400/60 `} type={type2} placeholder={placeholder2} readOnly={readonly2} />
       </div>
  ...
  if(!double){
    return (
      <div className='flex flex-col justify-center my-4 gap-2'>
       ...
       <input name={name} onChange={onChange} value={value} onBlur={onBlur} className={`w-full pl-[15px] text-sm h-10 py-2 border border-solid border-lenssisStroke rounded-[5px] max-w-${inputWidth}  focus:outline-1 focus:outline-[#ABC8DF] placeholder-gray-400/60`} type={type}  placeholder={placeholder} readOnly={readonly} />
       </div>
       {isBlur && !isValidValue && <FormErrorMessage errorText={errorText} errorTitle={placeholder}  textSize='xs' titleSize='sm'  /> }
      </div>
      )  
