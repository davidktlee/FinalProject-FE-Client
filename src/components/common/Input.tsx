...
const Input = ({margin,label,placeholder,type,double, flexDirection,type2,placeholder2,isRequired,readonly,inputWidth,inputHeight,labelColor,labelBold,name,name2,readonly2,value,value2,onChange,validators,errorText}:InputProps) => {
  ...
  if(!double){
    return (
      <div className={`flex flex-col justify-center ${margin ? margin : 'my-4'} gap-2`}>
       ...
       <input name={name} onChange={onChange} value={value} onBlur={onBlur} className={`w-full pl-[15px] text-sm h-10 py-2 border border-solid border-lenssisStroke rounded-[5px] ${inputWidth ? inputWidth : 'max-w-[400px]'}  focus:outline-1 focus:outline-[#ABC8DF] placeholder-gray-400/60`} type={type}  placeholder={placeholder} readOnly={readonly} />
       </div>
       {isBlur && !isValidValue && <FormErrorMessage errorText={errorText} errorTitle={placeholder}  textSize='xs' titleSize='sm'  /> }
      </div>
      )  
