...
const Radio = ({title,value,onChange,children}:RadioProps) => {
  return (
    <div  className='flex items-center gap-1 text-xs w-[40%] justify-start xs:justify-center  xs:w-full xs:text-base'><input
          type="radio"
          name="payment-method"
          value={value}
        ...
        /><label className="font-bold text-lenssisGray">{title || children}</label></div>
  );
};
