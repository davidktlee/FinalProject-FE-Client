...
### - prefetch를 이용한 메인 페이지 전체 상품 조회 및 새로운 상품 조회
  - 메인의 상품들을 page 별로 불러오고 총 count를 받아서 pagination을 구현함과 동시에 React-Query의 prefetch 로직을 이용해서 사용자가 2페이지 눌렀을 때 상품이 로딩되는 시간을 단축시켰고 이전 데이터도 캐시에 유지시킴으로써 또 다시 서버 호출이 가지 않도록 구현했습니다.
  - 렌즈 사이트 특성은 이미지가 많고 일본 사이트 특성은 메인에 많은 정보들을 보여주려고 하는데 그러다 보니 메인 페이지부터 loading이 느리게 온 다는 느낌이 들어서 lazy loading 을 통해 메인 페이지에 첫 화면 불러올 때의 부담을 줄였습니다.
### - 상품의 색상 별 이미지 조회
